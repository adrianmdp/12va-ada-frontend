...
const juego = () => {
  ...
  while (eleccion !== fin) {
    ...
    let resultado: string = opciones[random];
    eleccion = prompt(
      `Ingrese (${resultado}) cara o cruz. Para terminar el juego ingrese ${fin}.`
    );
...
const sonIguales = (a, b) => {
  ...
  return true;
  //   return (
  //     a.length === b.length && a.pepe((element, index) => element === b[index])
  //   );
};

console.log(sonIguales([5, 5, 2, 2], [7, 5, 2, 6]));
console.log(sonIguales([6, 6, 6, 6], [6, 6, 6, 6]));
